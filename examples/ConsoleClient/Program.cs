...
namespace ConsoleClient
{
    static class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                if (TryGetGdsSettings(settings, out UnifiedAutomation.UaSchema.GdsSettings gdsSettings, out string gdsFolder))
                {
                    ...
                    clientConfigurationInMemory.SetSecurity(PlatformUtils.CombinePath(PlatformUtils.IsWindows() ? "%CommonApplicationData%" : "%LocalApplicationData%", "UnifiedAutomation", "UaSdkNetBundlePubSubBinary", gdsFolder), "CN=ConsoleClient/O=UnifiedAutomation/DC=localhost");
                    /// [GDS Security]
        ...
        public static IConfiguration LoadInMemoryConfiguration()
        {
            ...
            configuration.SetSecurity(PlatformUtils.CombinePath(PlatformUtils.IsWindows() ? "%CommonApplicationData%" : "%LocalApplicationData%", "UnifiedAutomation", "UaSdkNetBundlePubSubBinary", "pkiclient"), "CN=ConsoleClient/O=UnifiedAutomation/DC=localhost");
        ...
        static UnifiedAutomation.UaSchema.TraceSettings TraceSettingsFromConfiguration(IClientConfiguration configuration)
        {
            if (configuration.TraceSettings != null)
            {
                return new UnifiedAutomation.UaSchema.TraceSettings()
                {
                    ...
                    TraceFile = PlatformUtils.CombinePath(PlatformUtils.IsWindows() ? "%CommonApplicationData%" : "%LocalApplicationData%", "UnifiedAutomation", "logs", "UaSdkNetBundlePubSubBinary", configuration.TraceSettings.TraceFile),
            ...
            return new UnifiedAutomation.UaSchema.TraceSettings()
            {
                ...
                TraceFile = PlatformUtils.CombinePath(PlatformUtils.IsWindows() ? "%CommonApplicationData%" : "%LocalApplicationData%", "UnifiedAutomation", "logs", "UaSdkNetBundlePubSubBinary", "ConsoleClient.txt"),
                ModuleSettings = new UnifiedAutomation.UaSchema.ModuleTraceSettings[]
                    {
                    new UnifiedAutomation.UaSchema.ModuleTraceSettings() { ModuleName = "UnifiedAutomation.Stack" },
                    new UnifiedAutomation.UaSchema.ModuleTraceSettings() { ModuleName = "UnifiedAutomation.Client" }
