...
namespace UnifiedAutomation.ClientGettingStarted
{
    ...
    /// </summary>
    public partial class CreateEventMonitoredItemsDlg : Form
    {
        /// <summary>
        ...
        /// </summary>
        public CreateEventMonitoredItemsDlg()
        {
            InitializeComponent();
        ...
        private ListViewItem AddItem(NodeId nodeId)
        {
            ...
            monitoredItem.Filter.SetDefault();
        ...
        private void SelectNodeBTN_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dialog.Size = new Size(m_parent.Width, Math.Max(400, m_parent.Height));
                dialog.Location = this.Location;

                uint nodeClassMask = (uint)(NodeClass.ObjectType | NodeClass.Variable | NodeClass.Object);
                NodeId nodeId = dialog.ShowDialog(m_parent.Session, ObjectIds.Server, ReferenceTypeIds.HasEventSource, nodeClassMask);

                if (nodeId != null)
                {
                    AddItem(nodeId).Selected = true;
        ...
        private void MonitoredItemsLV_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                EventFilterBTN.Enabled = MonitoredItemsLV.SelectedItems.Count != 0;
            }
            catch (Exception exception)
            {
                ExceptionDlg.ShowInnerException(this.Text, exception);
        ...
        private void EventFilterMI_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                EventMonitoredItem monitoredItem = (EventMonitoredItem)MonitoredItemsLV.SelectedItems[0].Tag;
                ...
                UaClient.ItemEventFilter filter = dialog.ShowDialog(monitoredItem.Filter);

                if (filter != null)
                {
                    foreach (ListViewItem item in MonitoredItemsLV.SelectedItems)
                    {
                        monitoredItem = (EventMonitoredItem)MonitoredItemsLV.SelectedItems[0].Tag;
