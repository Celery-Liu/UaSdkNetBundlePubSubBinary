...
namespace UnifiedAutomation.Demo
{
    ...
    public class RandomSource : IRandomSource
    {
        ...
        public void NextBytes(byte[] bytes, int offset, int count)
        {
            ...
            else
            {
                ...
                Buffer.BlockCopy(buffer, 0, bytes, offset, count);
            }
        }
    ...
    public class DataGenerator
    {
        ...
        /// <summary>
        /// The minimum length for generated arrays.
        /// </summary>
        public int MinArrayLength
        {
            get;
            set;
        }
        ...
        /// <summary>
        /// The minmum length for generated strings.
        /// </summary>
        ...
        /// </summary>
        public Variant GetRandom(NodeId dataType, int valueRank, IList<uint> arrayDimensions, ITypeTable typeTree)
        {
            BuiltInType expectedType = TypeUtils.GetBuiltInType(dataType, typeTree);
            ...
            // create an array.
            Array output = TypeUtils.CreateArray(expectedType, actualDimensions);
            ...
            for (int ii = 0; ii < length; ii++)
            {
                ...
                {
                    output.SetValue(value.Value, indexes);
                }
            }

            // return array value.
            return new Variant(output, null);
        }
        ...
        public Array GetRandomArray(BuiltInType expectedType, bool useBoundaryValues, int length, bool fixedLength)
        {
            switch (expectedType)
            {
                ...
                case BuiltInType.XmlElement: { return GetRandomArray<XmlString>(useBoundaryValues, length, fixedLength); }
                ...
                case BuiltInType.Variant:
                {
                    ...
                    if (length > 0)
                    {
                        ...
                        {
                            var type = (BuiltInType)GetRandomRange((int)BuiltInType.Boolean, (int)BuiltInType.LocalizedText);
                            variants[ii] = new Variant(GetRandom(type), null);
                        }
                ...
                {
                    int[] ints = GetRandomArray<int>(useBoundaryValues, length, fixedLength);
                    Model.HeaterStatus[] ret = new Model.HeaterStatus[ints.Length];
                    for (int ii=0; ii<ints.Length; ii++)
                    {
                        ret[ii] = (Model.HeaterStatus)(ints[ii] % 3);
                    }
                    return ret;
                }
        ...
        public T GetRandom<T>(bool useBoundaryValues)
        {
            ...
            return (T)GetRandom(typeof(T)).Value;
        }
        ...
        public T[] GetRandomArray<T>(bool useBoundaryValues, int length, bool fixedLength)
        {
            ...
            for (int ii = 0; ii < value.Length; ii++)
            {
                ...
                {
                    element = GetRandom(typeof(T)).Value;
                }
        ...
        public byte[] GetRandomByteString()
        {
            ...
            if (length == 0)
            {
                // return ByteString instead of empty ByteString. Workaround for CTT 200.
                return null;
            }
        ...
        /// <summary cref="GetRandom(Type)" />
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Design", "CA1024:UsePropertiesWhereAppropriate")]
        public XmlString GetRandomXmlElement()
        {
            #if !SILVERLIGHT
            ...
            XmlElement element = document.CreateElement(
                ...
                String.Format("http://{0}", CreateString(locale1, true)));
        ...
        public ExpandedNodeId GetRandomExpandedNodeId()
        {
            ...
            return new ExpandedNodeId(nodeId.IdType, nodeId.Identifier, m_namespaceUris[nodeId.NamespaceIndex], serverIndex);
        }
        #endregion
        ...
        private Variant GetRandomVariant(BuiltInType builtInType, bool isArray)
        {
            ...
            if (length >= 0)
            {
                switch (builtInType)
                {
                    ...
                    case BuiltInType.XmlElement: { return new Variant(GetRandomArray<XmlString>(true, length, true)); }
        ...
        private static readonly BoundaryValues[] s_AvailableBoundaryValues = new BoundaryValues[]
        {
            ...
            new BoundaryValues(typeof(NodeId), null, NodeId.Null, new NodeId(Guid.Empty, 0), new NodeId(String.Empty, 0), new NodeId(new byte[0], 0) ),
            new BoundaryValues(typeof(ExpandedNodeId), null, ExpandedNodeId.Null, new ExpandedNodeId(Guid.Empty, 0), new ExpandedNodeId(String.Empty, 0), new ExpandedNodeId(new byte[0], 0) ),
        ...
        /// </summary>
        private static SortedDictionary<string, string[]> LoadStringData()
        {
            ...
            try
            {
                ...
                {
                    if (name.EndsWith(".LocalizedData.txt", StringComparison.OrdinalIgnoreCase))
                    {
                        istrm = System.Reflection.Assembly.GetExecutingAssembly().GetManifestResourceStream(name);
                        break;
                    }
                }
        ...
        {
            return GetRandom(TypeInfo.Construct(expectedType).BuiltInType);
        }
