...
namespace YourCompany.GettingStarted
{
    ...
    public class UnderlyingSystem : IDisposable
    {
        ...
        {
            BlockProperty property = FindProperty(blockAddress, tag);
        ...
        public bool Write(int blockAddress, int tag, object value)
        {
            ...
            if (property != null)
            {
                ...
                {
                    RaiseEvent(blockAddress, property.Offset, (int)value);
                }

                if (property.History != null)
                {
                    property.History.Insert(new Variant(value, null), StatusCodes.Good, DateTime.UtcNow);
                }
        ...
        public StatusCode Start(int blockAddress)
        {
            ...
            if (tag >= 0)
            {
                RaiseEvent(blockAddress, tag, 1);
                Write(blockAddress, tag, (int)1);
            }
        ...
        public StatusCode Stop(int blockAddress)
        {
            ...
            if (tag >= 0)
            {
                RaiseEvent(blockAddress, tag, 0);
                Write(blockAddress, tag, (int)0);
            }
        ...
        /// <summary>
        /// Gets the history data source for the tag.
        /// </summary>
        /// <param name="blockAddress">The block address.</param>
        /// <param name="tag">The tag.</param>
        /// <returns>The history data source for the tag; null if there is no history.</returns>
                ...
                for (int ii = 0; ii < m_configuration.Controllers.Length; ii++)
                {
                    ...
                    if (controller.Properties != null)
                    {
                        for (int jj = 0; jj < controller.Properties.Length; jj++)
                        {
                            ...
                            if (property.Historizing)
                            {
                                bp.History = new InMemoryHistoryDataSource();
                                bp.HistoryConfiguration = new HistoricalDataConfigurationModel();
                                bp.HistoryConfiguration.StartOfArchive = bp.HistoryConfiguration.StartOfOnlineArchive = DateTime.UtcNow;
                                bp.HistoryConfiguration.Stepped = false;
                                bp.HistoryConfiguration.AggregateConfiguration = new AggregateConfigurationModel();
                                bp.HistoryConfiguration.AggregateConfiguration.PercentDataBad = 100;
                                bp.HistoryConfiguration.AggregateConfiguration.PercentDataGood = 100;
                                bp.HistoryConfiguration.AggregateConfiguration.TreatUncertainAsBad = true;
                                bp.HistoryConfiguration.AggregateConfiguration.UseSlopedExtrapolation = false;
                            }

                            data.Properties.Add(bp);
                    ...
                    foreach (var blockAddress in m_blocks)
                    {
                        ...
                        if (active != 0)
                        {
                            for (int ii = 0; ii < blockAddress.Value.Properties.Count - 1; ii++)
                            {
                                ...
                                var history = blockAddress.Value.Properties[ii].History;
        ...
        {
            BlockStateChangedEventHandler callback = BlockStateChanged;
            ...
            if (callback != null)
            {
                ...
                lock (m_lock)
                {
                    oldState = BitConverter.ToInt32(m_registers, blockAddress + tag);
                    blockName = m_blocks[blockAddress].Name;
                }
